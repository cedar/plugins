...
========================================================================================================================
    ...
    File:        SerialOrderRecruiting.cpp
...
// parameters
        ...
        _mSigmoid
                (
                        new cedar::dyn::SerialOrderRecruiting::SigmoidParameter
                                (
                                        this,
                                        "sigmoid",
        ...
        _mUseDynamicExtension(new cedar::aux::BoolParameter(this,"use dynamic extension",false))
{
  ...
  this->registerFunction("RecruitAGroup", boost::bind(&SerialOrderRecruiting::createRecruit, this), false);
...
void cedar::dyn::SerialOrderRecruiting::numberOfOrdinalPositionsChanged()
{
  ...
  if (new_number_of_positions > old_number_of_positions)
  {
    ...
    for (unsigned int i = old_number_of_positions; i < new_number_of_positions; ++i)
    {
      ...
      mOrdinalNodes.push_back(cedar::aux::MatDataPtr(
              new cedar::aux::MatData(cv::Mat::ones(1, 1, CV_32F) * _mOrdinalNodeRestingLevel->getValue())));
      mMemoryNodes.push_back(cedar::aux::MatDataPtr(
              new cedar::aux::MatData(cv::Mat::ones(1, 1, CV_32F) * _mMemoryNodeRestingLevel->getValue())));
...
void cedar::dyn::SerialOrderRecruiting::eulerStep(const cedar::unit::Time &time)
{
  ...
  for (unsigned int i = 0; i < mOrdinalNodes.size(); ++i)
  {
    ...
    if (this->mCosSignalInput)
    {
      ...
      //Add the Dynamic Extension
      if(this->_mUseDynamicExtension->getValue())
      {
//        std::cout<<">>We have an Input and Use Dynamic Extension!"<<std::endl;
        if(this->mCosSignalInput->getData().at<float>(0,0) > 0.5)
        {
//          std::cout<<"--CoS is above 0.5"<<std::endl;
          if(!_mRecruited)
          {
            auto numOrdinals = _mNumberOfOrdinalPositions->getValue();
            _mNumberOfOrdinalPositions->setValue(numOrdinals+1);
...
void cedar::dyn::SerialOrderRecruiting::reset()
{
  for (unsigned int i = 0; i < mOrdinalNodes.size(); ++i)
  {
    cv::Mat &d = this->mOrdinalNodes.at(i)->getData();
...
cedar::proc::DataSlot::VALIDITY cedar::dyn::SerialOrderRecruiting::determineInputValidity
        (
